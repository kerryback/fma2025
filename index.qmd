...
<br>
::: {.quote-box}
Create a Jupyter notebook for mean-variance optimization. Prompt the user for the number of assets, their means, standard deviations, and correlations and  risk-free rate.  Only ask for the minimum number of correlations required to compute the correlation matrix.  Compute and display the tangency portfolio, ASSUMING SHORT SALES ARE ALLOWED. 
:::


## {background-video="columbo_final.webm" background-video-loop=true background-video-muted=false}

## Adding a tool to Claude Desktop

- Anthropic created Model Context Protocol (MCP) for adding tools to chatbots 
- Recently improved configuration method for users (mcpb files)
